package red.mohist.common.async.concurent;
...
public class ThreadSafeList<E> extends Vector<E> {
    ...
    public void add(int index, E element) {
        if (checkThread()) {
            postPrimaryThread(() -> super.add(index, element));
            if (warn) {
                FMLLog.log.debug(new UnsupportedOperationException(message));
            }
            return;
    ...
    public boolean remove(Object o) {
        if (checkThread()) {
            postPrimaryThread(() -> super.remove(o));
            if (warn) {
                FMLLog.log.debug(new UnsupportedOperationException(message));
            }
    ...
    public synchronized E remove(int index) {
        if (checkThread()) {
            ...
            postPrimaryThread(() -> super.remove(removeE));
            if (warn) {
                FMLLog.log.debug(new UnsupportedOperationException(message));
            }
            return removeE;
    ...
    public synchronized boolean addAll(Collection<? extends E> c) {
        if (checkThread()) {
            postPrimaryThread(() -> super.addAll(c));
            if (warn) {
                FMLLog.log.debug(new UnsupportedOperationException(message));
            }
            return true;
    ...
    public synchronized boolean addAll(int index, Collection<? extends E> c) {
        if (checkThread()) {
            postPrimaryThread(() -> super.addAll(index, c));
            if (warn) {
                FMLLog.log.debug(new UnsupportedOperationException(message));
            }
            return true;
    ...
    public synchronized void addElement(E obj) {
        if (checkThread()) {
            postPrimaryThread(() -> super.addElement(obj));
            if (warn) {
                FMLLog.log.debug(new UnsupportedOperationException(message));
            }
            return;
